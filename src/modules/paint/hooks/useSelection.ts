...
const useSelection = () => {
    ...
    const drawSelectionRect = useCallback((x: number, y: number, w: number, h: number) => {
        ...
        if(gridDisplayMode !== 'none') {
            drawGrid(overlay.canvas.width, overlay.canvas.height);
        }
        ...
        overlay.restore();
        renderViewport();
    }, [replacementContextRef, snap, gridDisplayMode, drawGrid, renderViewport]);
    ...
    const clearSelectionOverlay = useCallback(() => {
        ...
        renderViewport();
    }, [replacementContextRef, gridDisplayMode, drawGrid, renderViewport]);
    ...
    const stopSelection = useCallback(() => {
        ...
        renderViewport();
    }, [clearSelectionOverlay, contextRef, copyCanvasToClipboard, renderViewport]);
